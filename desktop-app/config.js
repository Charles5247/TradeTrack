/**
 * TracKasuwa Desktop — runtime configuration.
 *
 * TracKasuwa is an offline-first Progressive Web App (IndexedDB + a
 ...
 *   1. TracKasuwa_APP_URL environment variable (set by the installer /
 *      IT admin for a specific deployment)
 *   2. A `TracKasuwa-config.json` file next to the packaged app (lets a
 ...
 * copy of the domain. If TracKasuwa.com (or any other domain) is
...
// This is TracKasuwa's own hosted deployment origin (no trailing slash,
// no path). Do NOT assume TracKasuwa.com is secured yet — keep
// TracKasuwa.ng here until told otherwise. Changing this alone (and
// re-running `npm run build:win`) repoints the installer; it can also
// be overridden without rebuilding via TracKasuwa_APP_URL or
// TracKasuwa-config.json (see getAppUrl() below).
// ─────────────────────────────────────────────────────────────────
const DEFAULT_APP_URL = "https://TracKasuwa.ng";
...
function readConfigFile() {
  try {
    ...
    const candidates = [
      path.join(process.resourcesPath || "", "TracKasuwa-config.json"),
      path.join(__dirname, "TracKasuwa-config.json"),
    ];
...
function getAppUrl() {
  if (process.env.TracKasuwa_APP_URL) {
    return process.env.TracKasuwa_APP_URL;
  }
...
function getVersionCheckUrl() {
  if (process.env.TracKasuwa_VERSION_URL) {
    return process.env.TracKasuwa_VERSION_URL;
  }
  try {
...
// TracKasuwa_APP_URL or TracKasuwa-config.json's `appUrl` continues to

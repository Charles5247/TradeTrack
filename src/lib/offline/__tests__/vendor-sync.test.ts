...
it("sends vendor parents first and does not burn child/sale retries when the parent fails", async () => {
  ...
  expect(
    queue
      .filter((row) => row.table_name !== "vendor_transactions")
      .every((row) => row.retry_count === 0),
  ).toBe(true);
});
...
it("retries parent and mirrored sale without overwriting server payment state", async () => {
  ...
  client.from.mockImplementation((table: string) => ({
    upsert: (
      ...
      if (table === "vendor_transactions" && fail) {
        fail = false;
  ...
  await engine.pushChanges();
  server.set(`vendor_transactions:${vendor.id}`, {
    status: "completed",
    amount_paid: 30,
  });
  await engine.pushChanges();
  expect(server.size).toBe(3);
  expect(server.get(`vendor_transactions:${vendor.id}`)).toMatchObject({
    status: "completed",
    amount_paid: 30,
  });
  const db = await getDB();
  const saleQueue = (await db.getAll("sync_queue")).find(
    (row) => row.table_name === "sales",
  );
  server.set(`sales:${saleQueue.record_id}`, { payment_status: "paid" });
  await db.put("sync_queue", { ...saleQueue, status: "pending" });
  await engine.pushChanges();
  expect(server.get(`sales:${saleQueue.record_id}`)).toEqual({
    payment_status: "paid",
  });
});

it.each(["vendor_transactions", "vendor_transaction_items"])(
  "blocks UPDATE for %s",
  async (table_name) => {
    await engine.executeSyncOperation(client, {
      table_name,
      operation: "UPDATE",
      payload: {},
    } as SyncQueueRecord);
...
it("claims the latest pending inventory payload even when refreshed after the flush snapshot", async () => {
  ...
  client.from.mockImplementation((table: string) => {
    ...
    return {
      upsert: async () => {
        if (table === "vendor_transactions" && !refreshed) {
          refreshed = true;
          await persistOfflineVendorTransaction(payload);
        }
        return { error: null };
...
it("pulls organization-scoped data and only vendor-linked sales, preserving pending rows", async () => {
  ...
  expect(calls).not.toContainEqual(["deleted_at", null]);
  const db = await getDB();
  expect((await db.get("vendor_transactions", vendor.id)).vendor_name).toBe(
    "Vendor",
  );

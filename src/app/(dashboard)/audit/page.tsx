        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs tt-muted">{t.audit.to_separator}</span>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-0">
            <EmptyState icon={ClipboardList} title={t.audit.no_logs} />
          </CardContent>
        </Card>
      ) : (
        <Card>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {logs.map((log) => (
                  <TableRow key={log.id}>
                    <TableCell className="text-xs tt-muted whitespace-nowrap">
                      {formatDateTime(log.created_at)}
                    </TableCell>
                    <TableCell>
                      <div>
                        <p className="text-sm font-medium">{(log.user as { full_name?: string } | null)?.full_name || 'System'}</p>
                        <p className="text-xs tt-muted">{(log.user as { email?: string } | null)?.email}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge
                    ...
                    <TableCell className="text-xs tt-mono">{log.ip_address || '—'}</TableCell>
                    <TableCell>
                      {log.old_values && log.new_values ? (
                        <span className="text-xs tt-muted">
                          {t.audit.fields_changed.replace('{count}', String(Object.keys(log.new_values).length))}
                        </span>
                      ) : log.new_values ? (
                        <span className="text-xs tt-muted">{t.audit.new_record}</span>
                      ) : (
                        <span className="text-xs tt-muted">—</span>
                      )}
                    </TableCell>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-3">
                <div><span className="tt-muted">{t.audit.timestamp}:</span><br /><span className="font-medium">{formatDateTime(viewLog.created_at)}</span></div>
                <div><span className="tt-muted">{t.audit.action}:</span><br />
                  <Badge variant={(actionColors[viewLog.action] || 'outline') as Parameters<typeof Badge>[0]['variant']}>{viewLog.action}</Badge>
                </div>
                <div><span className="tt-muted">{t.audit.user}:</span><br /><span className="font-medium">{(viewLog.user as { full_name?: string } | null)?.full_name}</span></div>
                <div><span className="tt-muted">{t.audit.resource}:</span><br /><span className="font-medium capitalize">{viewLog.resource_type}</span></div>
                <div><span className="tt-muted">{t.audit.ip_address}:</span><br /><span className="tt-mono text-xs">{viewLog.ip_address || '—'}</span></div>
                <div><span className="tt-muted">{t.audit.browser}:</span><br /><span className="text-xs truncate">{viewLog.user_agent?.split(' ')[0] || '—'}</span></div>
              </div>
              {viewLog.reason && (
                <div><span className="tt-muted">{t.audit.reason}:</span><br /><span className="font-medium">{viewLog.reason}</span></div>
              )}
              {viewLog.old_values && (
                <div>
                  <p className="tt-muted mb-1">{t.audit.previous_values}:</p>
              ...
              {viewLog.new_values && (
                <div>
                  <p className="tt-muted mb-1">{t.audit.new_values}:</p>
                  <pre className="bg-muted rounded p-3 text-xs overflow-auto">

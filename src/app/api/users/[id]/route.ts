/**
 * TracKasuwa - Individual User Management API Routes
  ...
  try {
    ...
    if (isBusinessOwner) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Forbidden: Cannot modify users outside your own organization",
          },
          { status: 403 },
        );
      }
      if (
        targetUser.role === "business_owner" ||
        targetUser.role === "platform_owner"
      ) {
        return NextResponse.json(
          {
            error:
              "Forbidden: Business Owners can only manage admin or cashier users",
          },
          { status: 403 },
        );
      }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Forbidden: Business Owners can only assign admin or cashier roles",
        },
        { status: 403 },
      );
    }
    ...
    if (status !== undefined)
      profileUpdates.status = status as UserRow["status"];
    ...
    if (isBusinessOwner) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Forbidden: Cannot delete users outside your own organization",
          },
          { status: 403 },
        );
      }
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Forbidden: Business Owners can only delete admin or cashier users",
          },
          { status: 403 },
        );
      }

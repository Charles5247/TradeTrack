/**
 * TracKasuwa — Merchant Onboarding API
...
import {
  createMerchantVirtualAccount,
  ZainpayNotConfiguredError,
} from "@/lib/zainpay";
  ...
  try {
    ...
    if (!business_name || !contact_name || !contact_email) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: business_name, contact_name, contact_email",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      if (authErr || !authData?.user) {
        throw new Error(
          authErr?.message || "Failed to create business owner account",
        );
      }
      ...
      if (merchantErr || !merchant) {
        throw new Error(
          merchantErr?.message || "Failed to create merchant record",
        );
      }
      ...
      try {
        ...
        if (vacctErr instanceof ZainpayNotConfiguredError) {
          ...
          console.error(
            "[merchants/onboard] Zainpay virtual account creation failed (non-fatal):",
            vacctErr,
          );
        }
      }

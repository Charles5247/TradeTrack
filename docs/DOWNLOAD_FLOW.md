# TracKasuwa — Download & Distribution Flow

How a trader actually gets TracKasuwa onto their device, and how the
...
TracKasuwa is, first and foremost, an **offline-first Progressive Web App
...
the existing offline-first PWA rather than reimplement it — TracKasuwa
...
- **`page.tsx`** — Server Component. Reads the `TracKasuwa_*` env vars
...
- **`download-client.tsx`** — Client Component. Detects the visitor's OS
  ...
  1. **Windows Desktop** — direct `.exe` download button (only rendered if
     `TracKasuwa_WINDOWS_DOWNLOAD_URL` is set; otherwise shows "Download
  ...
  2. **Android** — a native "Install App (Chrome)" PWA-install button
     ...
     _and_ a direct `.apk` download button (only rendered if
     `TracKasuwa_ANDROID_DOWNLOAD_URL` is set).
  ...
  4. **iOS & macOS** — always shows a "Coming Soon" badge and an honest
     ...
     use TracKasuwa directly in Safari as a stand-in.
...
(`export const dynamic = 'force-dynamic'`) endpoint that the _installed_
...
both shells only _notify_ the user that an update exists; the user must
...
Right now, `TracKasuwa_WINDOWS_DOWNLOAD_URL` and
`TracKasuwa_ANDROID_DOWNLOAD_URL` are blank in `render.yaml` / `.env.example`
...
--win --x64 --publish never` → `dist/TracKasuwa-Setup-<version>.exe`) or
   `android-app/README.md` (`gradle assembleRelease` → sign with a securely
   stored production keystore → `TracKasuwa-<version>.apk`).
...
3. **Set the env var** on Render (or wherever you deploy — see
   ...
   - `TracKasuwa_WINDOWS_DOWNLOAD_URL=https://.../TracKasuwa-Setup-1.0.0.exe`
   - `TracKasuwa_ANDROID_DOWNLOAD_URL=https://.../TracKasuwa-1.0.0.apk`
   - Bump `TracKasuwa_WINDOWS_LATEST_VERSION` / `TracKasuwa_ANDROID_LATEST_VERSION`
...
For Android specifically, also set `TracKasuwa_ANDROID_UNKNOWN_SOURCES_HELP_URL`
...
- **No macOS build** — `electron-builder`'s `package.json` config only
  ...
  already scheduled.
- **No automatic update install** — both shells only _check_ for updates
